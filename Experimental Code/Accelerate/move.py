...
def accelToExponential(Lpower, Rpower, numberOfSteps, deltaTime, base=7):
    global current_right_power, current_left_power
    ...
    inital_exponent_left = (delta_left*(left_exponent_term))
    inital_exponent_right  = (delta_right*(right_exponent_term))
    counter = 0

    while(counter < numberOfSteps):
        current_left_power =  inital_left_power + ((inital_exponent_left*(left_exponent_term**counter))/base)
        current_right_power = inital_right_power + ((inital_exponent_right*(right_exponent_term**counter))/base)
